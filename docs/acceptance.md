...
- numerical output, log, event, artifact, cumulative runtime, retry, resume,
  optional total-model-token, and provider-cost bounds produce documented
  truncation or terminal outcomes; configured total-token accounting includes
  input, output, cache-read, and cache-write traffic;
- whichever of token, provider-reported cost, run-runtime, or attempt-runtime
  pressure first reaches 70% and 90% persists one unified receipt per attempt
  stage and queues
